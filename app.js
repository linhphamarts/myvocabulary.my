...
function chooseEnglishVoice(){if(!('speechSynthesis'in window))return null;const voices=window.speechSynthesis.getVoices();if(!voices.length)return speechState.voice;const english=voices.filter(v=>/^en(?:-|$)/i.test(v.lang));const preferred=english.find(v=>/^en-US$/i.test(v.lang)&&v.localService)||english.find(v=>/^en-US$/i.test(v.lang))||english.find(v=>v.localService)||english[0]||voices.find(v=>v.default)||voices[0]||null;if(preferred)speechState.voice=preferred;return speechState.voice}
function primeSpeech(){if(!('speechSynthesis'in window)||speechState.primed)return;speechState.primed=true;chooseEnglishVoice();try{const warm=new SpeechSynthesisUtterance(' ');warm.lang=speechState.voice?.lang||'en-US';if(speechState.voice)warm.voice=speechState.voice;warm.volume=0;window.speechSynthesis.speak(warm)}catch{}}
if('speechSynthesis'in window){chooseEnglishVoice();window.speechSynthesis.addEventListener?.('voiceschanged',chooseEnglishVoice)}
function speak(text){if(!('speechSynthesis'in window)||typeof SpeechSynthesisUtterance==='undefined')return;const clean=cleanSpeechText(text);if(!clean)return;const synth=window.speechSynthesis,voice=chooseEnglishVoice();try{if(synth.speaking||synth.pending)synth.cancel()}catch{}const u=new SpeechSynthesisUtterance(clean);u.lang=voice?.lang||'en-US';if(voice)u.voice=voice;u.rate=.82;u.pitch=1;u.volume=1;speechState.utterance=u;const release=()=>{if(speechState.utterance===u)speechState.utterance=null};u.onend=release;u.onerror=release;try{synth.speak(u)}catch{release()}}
